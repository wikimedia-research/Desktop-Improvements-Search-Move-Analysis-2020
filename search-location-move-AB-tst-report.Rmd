...
table_caps(name = "CTR by search relative risk overall", caption = "How likely new search location users were to click on a search result during a search.")
...
figure_caps(name = "Total searches initiated", caption = "Total number of search sessions initiated by search location group and wiki.")
...
figure_caps(name = "Search Sessions Initiated Pre AB Test vs Post AB Test by Search Location Group", caption = "Search Sessions Initiated Pre AB Test vs Post AB Test by Search Location Group. We applied the log transformation to both the pre and post deployment data to address skew in the data caused by outlier Wikis ")
figure_caps(name = "BCDA ctr sessions overall", caption = "How likely new search location users were to click on a search result at least once in a session overall. A relative risk greater than 1 indicates the test group was more likely to complete a session, while a relative risk less than 1 indicates the test group was less likely to complete a session")
...
figure_caps(name = "BCDA ctr searches overall", caption = "How likely new search location users were to click on a search result during a search overall. A relative risk greater than 1 indicates the test group was more likely to complete a session, while a relative risk less than 1 indicates the test group was less likely to complete a session")
figure_caps(name = "CTR searches by wiki", caption = "Clickthrough rates of test groups, split by wiki.")
...
The new location (as shown in screenshot below) was also deployed as default for anonymous users on our early adopter wikis, and by preference for all other users.

![](Figures/search_header_move_example.png)
...
As an alternative, we conducted a pre and post deployment analysis comparing changes in search sessions initiated for the 6 test wikis to a set of wikis not included in the test from 6 October 2020 through 3 November 2020 (two weeks before and after deployment of the change).
...
search_sessions_ab_bywiki <- search_sessions %>%
    filter(logged_in_status == 'logged-in', #logged out users not included
          test_group == 'WikiProjectInTest',
          session_start >= '2020-10-20') %>% # AB test rereun on October 20
    mutate(wiki = case_when(
        wiki == 'euwiki' ~ "Basque Wikipedia", # some NULL records were recorded prior to deployment of isAnon field
        ...
        wiki == 'ptwikiversity' ~ 'Portuguese Wikipedia'
    )) %>%
...
```
The boxplot above shows the distribution of the search sessions initiated across all of the test wikis. We applied a logarithmic scale to the number of search sessions initiated (y-axis) to address the large discrepancies between data for each wiki.
...
To determine if the search location move had an impact, we calculated the total number of unique search sessions initiated by test Wiki and search location group for a two-week period before and after the AB test. We compared these results to search sessions initiated on similar size wikis over the same time period.
...
```{r search_sessions_daily_cap, echo=FALSE}
search_sessions_daily_cap <- format_caption(figure_caps, "Daily search sessions initiated")
```

```{r search_sessions_daily_plot, echo = FALSE, fig.cap=search_sessions_daily_cap}
...
![](Figures/daily_search_sessions_initiated_prepost.png)

The figure above shows the total daily search sessions initiated for all test wikis compared to similar wikis from 5 October 2020 through 4 November 2020. Post-AB test deployment search events are highlighted in yellow.  Overall, there appears to be no sudden change in daily search sessions initiated following deployment of the AB test on the test wikis. 
...
search_sessions_bywiki$test_group <- factor(search_sessions_bywiki$test_group, levels = c("NA", "WikiProjectNotInTest", "WikiProjectInTest"))
```
...
``` {r search_sessions_pre_post_plot, echo = FALSE, eval=FALSE, fig.cap=search_sessions_initiated_pre_post_cap }
p <- search_sessions_bywiki %>%
      ggplot( aes(x = log(pre), y = log(post), color = factor(test_group))) +
      geom_point(size = 5) +
...
![](Figures/search_sessions_initiated_pre_v_post.png)

We applied the log transformation to stabilize variance since the data are highly skewed by the outliers of French Wikipedia and Portuguese Wikiversity. Because the response variable is on the log scale, we then took the multiplicative effect of the treatment, and estimated the 95% confidence interval using the delta method.

We obtained the following estimates fitting the model to the data using a simple linear regression model  (`lm`) function:
...
While the estimate of the effect of the new search location is 2.4% increase in search sessions initiated, the 95% credible interval contains 1, meaning we do not have sufficient evidence to draw definitive conclusions.

Note that the effect from the search move is dampened because it was only deployed to 50% of logged-in users as part of the AB test. 
...
search_sessions_ab_ctr <- collect_ctr_data  %>%
  ...
  mutate(search_wiki = case_when(  #rename Wikis
          ...
          search_wiki == 'ptwikiversity' ~ 'Portuguese Wikipedia'),
...
We first reviewed the number of sessions with at least one clickthrough to a provided search result by wiki and overall.
...
bcda_sessions_ctr_overall <- BCDA::beta_binom(sessions_ctr_overall [2:1, 1:2]) #order factor levels in desc order for model
```
...
```
FIXME: NEED TO FORMAT PLOT ABOVE

Table 3 and Figure 5 shows the relative risk – how much more likely the new search location test group is to engage with the search results than the Control group. Overall, users that saw the new search location are 1.098 times more likely to click on at least 1 search result in a session compared to users that saw the header in the old location with a 95% confidence interval of (1.002, 1.204).  
...
On a per wiki basis, there were more completed search sessions (defined by sessions with at least 1 click to search result) for the new search location group than the old search location on Persian Wikipedia and French Wiktionary while the number of completed search sessions were lower on Hebrew Wikipedia, Basque Wikipedia, and French Wikipedia.

Portuguese Wikiversity also had more completed search sessions for the new search location; however, there were only 19 observed sessions during the test period so we are not able to reliably determine if the new search location had an impact.

Table 4 and Figure 7 shows the relative risk or how much more likely each respective test group is to click on a search result at least once in a session in the new search location (test group) than the old search location (control group). On Persian Wikipedia, users that saw the new header location are about 1.029 times more likely to click on at least one result during a session than users that saw the old header location, with a 95% confidence interval of 1.017-1.043. For the other 5 wikis in the test, the 95% credible intervals contain 1 indicating do not have sufficient evidence to draw definitive conclusions for these.
...
Users in both the new and the old search location groups have a similar average number of clicks per session, ranging from 0.94 to 1. There is a little more variance in the average number of searches per session for the test wikis and search location groups.

Hewbrew Wikipedia only averages between 1 to 2 searches per session for both search location groups while the other wikis average searches per session range from 3 to 7 searches per session. 
...
Table 5 and Figure 9 shows the relative risk – how much more likely users were to click on a search result during a search. Estimates indicate users are less likely to engage overall but we do not have sufficient evidence to draw definitive conclusions.
...
On a per wiki basis, there were more completed searches (defined by total number of clicks over total number of searches) for the new search location than the old search location on Hebrew, Persian, and Portuguese Wikipedia, while the completed search rates were lower on French Wikipedia, Basque Wikipedia, and French Wikitionary.

Table 6 and Figure 11 above shows the relative risk or how much more likely each respective test group is to click results in the new search location (test group) than the old search location (control group). On Hebrew Wikipedia, users that saw the new header location are about 1.082 times more likely to clickthrough during a search than users that saw the old header location, with a 95% confidence interval of 1.021-1.145. For the other 5 wikis in the test, we do not have sufficient evidence to infer any impact from the search location move.
